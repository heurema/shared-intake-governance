...
be referenced through multiple config files. It also rejects duplicate
`source_config_path` values so one config file cannot appear under multiple
logical source ids.
...
the same merge to each generated profile report. `run-source-config
--update-seen-state` and `smoke-source-config --update-seen-state` apply the
same merge to the one projection report they generate. `project-profiles
...
before any explicit update. Projection does not update state unless the update
flag is provided, and the core still does not define consumer publication or
dedupe policy.
