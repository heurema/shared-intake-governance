...
`inspect-source-config`, raw-root bounded raw metadata, run manifest, and
...
validating tracked source-set refs with unique source ids and unique source
config refs without executing them.
