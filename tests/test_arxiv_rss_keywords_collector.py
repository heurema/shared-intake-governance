...
EXPECTED_URL = (
    ...
    "search_query=all%3A%22coding+agent%22+AND+all%3Abenchmark"
    "&start=0&max_results=5&sortBy=submittedDate&sortOrder=descending"
)


class ArxivRssKeywordsCollectorTests(unittest.TestCase):
    def test_collect_writes_success_raw_body_and_metadata(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            paths = RuntimePaths(Path(tmp_dir) / "runtime")
            body = (
                b'<?xml version="1.0" encoding="UTF-8"?>'
                b"<feed><title>Arxiv coding agent benchmark</title></feed>"
            )
            seen_requests = []
            ...
            collector = ArxivRssKeywordsCollector(paths, http_get=fake_http_get)
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(
                ...
                paths.raw_failure_metadata_path(
                    "arxiv-code-agents", FETCHED_AT, "transport-error"
                ),
            )
            self.assertEqual(metadata["canonical_url"], EXPECTED_URL)
    ...
    def test_source_rejects_unsafe_or_unbounded_inputs(self):
        invalid_cases = [
            {"source_id": "../bad", "keywords": ["coding agent"], "max_results": 5},
            {"source_id": "arxiv code", "keywords": ["coding agent"], "max_results": 5},
            {"source_id": "arxiv-code", "keywords": [], "max_results": 5},
            {"source_id": "arxiv-code", "keywords": ["   "], "max_results": 5},
            {"source_id": "arxiv-code", "keywords": ["coding/agent"], "max_results": 5},
            {"source_id": "arxiv-code", "keywords": ["coding agent"], "max_results": 0},
            {"source_id": "arxiv-code", "keywords": ["coding agent"], "max_results": 101},
            {
