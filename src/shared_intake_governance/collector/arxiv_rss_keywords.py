"""Read-only arXiv keyword feed collector."""
    ...
    def request_url(self) -> str:
        params = {
            "search_query": " AND ".join(
                f"all:{_query_term(keyword)}" for keyword in self.keywords
            ),
            "start": "0",
...
@dataclass(frozen=True)
class ArxivRssKeywordsCollectionResult:
    source_id: str
    fetch_status: str
    canonical_url: str
    request_url: str
...
class ArxivRssKeywordsCollector:
...
def _query_term(keyword: str) -> str:
...
def _keyword(value: str) -> str:
    if not _KEYWORD.fullmatch(value.strip()):
        raise ValueError("keyword must be a safe arXiv query term")
    return value
