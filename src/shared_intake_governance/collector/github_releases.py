"""Read-only GitHub releases collector."""
...
class GitHubReleasesSource:
    ...
    def request_url(self) -> str:
        params = {"per_page": str(self.max_results)}
        return (
            f"{self.api_base_url.rstrip('/')}/repos/{self.owner}/{self.repo}"
            f"/releases?{urllib.parse.urlencode(params)}"
        )
...
@dataclass(frozen=True)
class GitHubReleasesCollectionResult:
    source_id: str
    fetch_status: str
    canonical_url: str
    request_url: str
...
class GitHubReleasesCollector:
    """Collect GitHub release evidence without sanitizing or projecting."""
