"""Read-only GitHub repository metadata collector."""
...
class GitHubRepoSource:
    ...
    def request_url(self) -> str:
        return f"{self.api_base_url.rstrip('/')}/repos/{self.owner}/{self.repo}"
...
@dataclass(frozen=True)
class GitHubRepoCollectionResult:
    source_id: str
    fetch_status: str
    canonical_url: str
    request_url: str
...
class GitHubRepoCollector:
    """Collect GitHub repository API evidence without sanitizing or projecting."""
    ...
    def collect(
        ...
        *,
        ...
        canonical_url = _canonical_url(source, response.body)
        error = _response_error(response.status, response.headers, response.body)
        metadata = {
...
def _canonical_url(source: GitHubRepoSource, body: bytes) -> str:
    if body:
        try:
...
def _response_error(
    ...
    if _rate_limited(status, headers):
        return {"kind": "rate_limited", "message": f"HTTP {status}", "retryable": True}
    return {
        "kind": "http_error",
...
def _github_path_segment(value: str, label: str) -> str:
    if not _GITHUB_PATH_SEGMENT.fullmatch(value):
