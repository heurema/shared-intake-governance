"""Read-only GitHub repository search collector."""
...
@dataclass(frozen=True)
class GitHubSearchCollectionResult:
    source_id: str
    fetch_status: str
    canonical_url: str
    request_url: str
...
class GitHubSearchCollector:
    """Collect GitHub repository search evidence without sanitizing or projecting."""
...
def _query(value: str) -> str:
    if not _QUERY.fullmatch(value.strip()):
        raise ValueError("query must be a safe GitHub search query")
    return value
